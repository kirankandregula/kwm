...
const SellingAdvice = () => {
  ...
  const { sellingRecommendations, generateSellingAdvice, numberOfStocksToSell } = useData();
        ...
        {sellingRecommendations.length > 0 && (
          <Box display="flex" alignItems="center" justifyContent="center" mt={2}>
            ...
            <Typography variant="body1" color="error" ml={1}>
              {`You have ${sellingRecommendations.length} stock${sellingRecommendations.length > 1 ? 's' : ''} to consider for selling.`}
            </Typography>
          </Box>
        )}
      </Box>
          ...
          {sellingRecommendations.length > 0 ? (
            sellingRecommendations.map((rec, index) => (
              <Box key={index} p={2} border={1} borderRadius={2} mb={1}>
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={3}>
                    <Typography variant="body1"><strong>Price:</strong> ${rec.price.toFixed(2)}</Typography>
                  </Grid>
                  <Grid item xs={3}>
                    <Typography variant="body1" color={rec.scopeToGrow <= 5 || rec.action === "Sell" ? "error" : "primary"}>
                      {rec.scopeToGrow <= 5 || rec.action === "Sell" ? "Sell Now" : "Hold"}
                    </Typography>
                  </Grid>
                </Grid>
            ...
            <Box display="flex" alignItems="center" mb={1}>
              ...
              <Typography variant="body1" color="warning.main" ml={1}>
                No stocks currently meet the criteria for selling. Please check back later.
              </Typography>
            </Box>
          )}
        </DialogContent>
