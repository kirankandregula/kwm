  ...
  return (
    <div className="card mx-auto mt-4 my-2 text-white text-center" style={{ width: "300px" }}>
      <div className="card-body bg-secondary" style={{ borderRadius: "10px" }}>
        ...
        <p className="card-text">Present Value: ₹{presentValue.toFixed(2)}</p>
        <p className="card-text">Quarterly Return: {quarterlyReturn}%</p>
        <p className="card-text">Billable Return (excluding 5%): ₹{billableReturn}</p>
        <p className="card-text">Billable Amount: ₹{billableAmount} to be paid</p>
      </div>
    </div>
  );
}
